...
load(file = paste0("C:/Users/tmobley/Box/Asian_Americans_dementia_data/",
                   "adi_adrd/analysis_data/county17_filtered.R"))
...
table(adi_tte_merge$main_dem_v1_sample, exclude = NULL) #4443 

adi_tte_maindem1 <- adi_tte_merge %>% filter(main_dem_v1_sample == 1)

#3. No missing adi --- from 180049
table(adi_tte_maindem1$kind_2000adi_staternk, exclude=NULL) #8617
...
#-- analytic sample should be n=171,432  

#---- Create data subset >=15 AA per county ----

asian_tte_maindem1 <- adi_tte_maindem1 %>% filter(asian == 1)

asian_county_n <- asian_tte_maindem1 %>%  
...
n <- nrow(adi_county15aa_tte %>% filter(asian == 1)) #18167 to 18110
n <- nrow(adi_county15aa_tte %>% filter(asian == 0)) #150485 to 149449
      ...
      ifelse(adi_county15aa_tte$countyid_masked=='06004',3,
         ifelse(adi_county15aa_tte$countyid_masked=='06673',4,
            ifelse(adi_county15aa_tte$countyid_masked=='06708',5,
               ifelse(adi_county15aa_tte$countyid_masked=='06720',6,NA))))))
  
  table(adi_county15aa_tte$countyid_collapse,exclude=NULL)
  table(adi_county15aa_tte$countyid_masked,adi_county15aa_tte$countyid_collapse,
        exclude=NULL)


#---- Asian immigrant enclave vars ----
adi_county15aa_tte %>% summarize(min_pct = min(pop_foreignborn_pct), 
                                 max_pct = max(pop_foreignborn_pct), 
                                 avg_pct = mean(pop_foreignborn_pct))

adi_county15aa_tte %>% summarize(min_pct = min(pop_asianalone_pct), 
                                  max_pct = max(pop_asianalone_pct), 
                                  avg_pct = mean(pop_asianalone_pct))
  
adi_county15aa_tte = mutate(adi_county15aa_tte, 
                            pop_foreignborn_sample = 
                            ntile(adi_county15aa_tte$pop_foreignborn_pct, 100),
                            pop_asianalone_sample = 
                              ntile(adi_county15aa_tte$pop_asianalone_pct, 100))
  
adi_county15aa_tte %<>% 
  mutate("pop_foreignborn_clean" = 
           (((adi_county15aa_tte$pop_foreignborn_pct*100) - 
               mean((adi_county15aa_tte$pop_foreignborn_pct*100)))/10),
        "pop_foreignborn_clean_s" = ((pop_foreignborn_sample - 50)/10),
        "pop_asianalone_clean" = 
          (((adi_county15aa_tte$pop_asianalone_pct*100) - 
              mean((adi_county15aa_tte$pop_asianalone_pct*100)))/10),
        "pop_asianalone_clean_s" = ((pop_asianalone_sample - 50)/10))

  # summary(adi_county15aa_tte$pop_foreignborn_sample)
  # summary(adi_county15aa_tte$pop_foreignborn_clean_s)
  # summary(adi_county15aa_tte$pop_foreignborn_pct)
  # summary(adi_county15aa_tte$pop_foreignborn_clean)
  # 
  # summary(adi_county15aa_tte$pop_asianalone_sample)
  # summary(adi_county15aa_tte$pop_asianalone_clean_s)
  # summary(adi_county15aa_tte$pop_asianalone_pct)
  # summary(adi_county15aa_tte$pop_asianalone_clean)

#---- Vars rescaled to 17 counties in analytic sample ----
county17_filtered %<>% select(blkgroupdensity_mi2, decile_kind2000adi_county,
                              quintile_kind2000adi_county)

adi_county15aa_merge <- merge(x=adi_county15aa_tte, y=county17_filtered,
                            by='blkgroupdensity_mi2')

  table(adi_county15aa_merge$decile_kind2000adi_county, exclude=NULL)
  table(adi_county15aa_merge$quintile_kind2000adi_county,
        adi_county15aa_merge$kind_2000adi_natrank,exclude=NULL)
  table(adi_county15aa_merge$quintile_kind2000adi_state,
        adi_county15aa_merge$kind_2000adi_natrank,exclude=NULL)
  
  table(adi_county15aa_merge$kind_2000adi_staternk,
        adi_county15aa_merge$kind_2000adi_natrank,exclude=NULL)
  
  summary(adi_county15aa_merge$pop_asianalone_pct)
  hist(adi_county15aa_merge$pop_asianalone_pct)
  
    
#---- Clean variables for analysis ----

adi_county15aa_tte<-adi_county15aa_merge

#Center/rescale block group density
summary(adi_county15aa_tte$blkgroupdensity_mi2, exclude = NULL)
adi_county15aa_tte$bgdensity_clean <- 
  (adi_county15aa_tte$blkgroupdensity_mi2 - 
     round(mean(adi_county15aa_tte$blkgroupdensity_mi2, na.rm = TRUE)))/1000

#Flag for end of follow up = dementia (to use in surv analysis)
adi_county15aa_tte$dem_end_flag <- 
  ifelse(adi_county15aa_tte$main_dem_v1_end_type == 'DEMENTIA', 1, 0)

#confirmed dem end flag variables are equivalent 
table(adi_county15aa_tte$main_dem_v1_end_dem_flag, 
      adi_county15aa_tte$dem_end_flag, exclude = NULL)

#Collapse employment indicators for MI
table(adi_county15aa_tte$employment_part_time_employed, 
      adi_county15aa_tte$employment_full_time_employed, exclude = NULL)

adi_county15aa_tte$full_part_employment <- 
  ifelse(adi_county15aa_tte$employment_full_time_employed == 1 | 
           adi_county15aa_tte$employment_part_time_employed == 1, 1, 0)

table(adi_county15aa_tte$full_part_employment, exclude = NULL)

#create ethnicity var with "Asian, unspecified" category and collapse white
adi_county15aa_tte$ethnicity_new <-
  ifelse(!is.na(adi_county15aa_tte$ethnicity_rev), 
         adi_county15aa_tte$ethnicity_rev,
         ifelse(is.na(adi_county15aa_tte$ethnicity_rev) & 
                  adi_county15aa_tte$asian == 0, 9,
                ifelse(is.na(adi_county15aa_tte$ethnicity_rev) &
                         adi_county15aa_tte$asian == 1, 11, NA)))

#check
table(adi_county15aa_tte$ethnicity_new, exclude = NULL)

#create education4 variable for Table 1 (note, we will write over this after MI)
table(adi_county15aa_tte$education_rev, exclude=NULL)

adi_county15aa_tte$education4 <- 
  as.factor(ifelse(adi_county15aa_tte$education_rev %in% c(1, 2), 1,
                ifelse(adi_county15aa_tte$education_rev == 3, 2, 
                    ifelse(adi_county15aa_tte$education_rev == 4, 3, 
                         ifelse(adi_county15aa_tte$education_rev %in% c(5,6),4,
                                NA)))))
table(adi_county15aa_tte$education_rev, adi_county15aa_tte$education4, 
      useNA = "ifany")
...
# ns <-adi_county15aa_tte %>% 
#   group_by(ethnicity, quintile_kind2000adi_state, dem_end_flag) %>% tally()
# #view(ns)
# 
# ns2<-adi_county15aa_tte %>% 
#   group_by(ethnicity, quintile_kind2000adi_county, dem_end_flag) %>% tally()
# #view(ns2)
# 
# ns3 <-adi_county15aa_tte %>% 
#   group_by(ethnicity, quartile_kind2000adi_nat, dem_end_flag) %>% tally()
# view(ns3)
# 
# ns4 <-adi_county15aa_tte %>% 
